...
Add `data-coreui-toggle="button"` to toggle a button's `active` state. Also add `aria-pressed` so assistive technologies announce the control as a toggle button. Use `aria-pressed="false"` for the default state. If you're pre-toggling a button, you must add the `.active` class **and** `aria-pressed="true"`.
...
<Callout color="info">
`aria-pressed` has no default value. A button without it is announced as a plain button, not as a toggle. As a safety net, CoreUI fills the attribute in on `DOMContentLoaded` for the toggle buttons the page already contains, reading the `.active` class for the value. That pass runs once and only over elements that can carry the state, so a button rendered afterwards, or one on a page whose scripts load after parsing, is not covered. Write the attribute in your markup, and keep it in step with `.active`: a value that disagrees with the class is left as the author wrote it.
</Callout>
